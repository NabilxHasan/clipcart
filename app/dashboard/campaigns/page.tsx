    ...
    <div className="space-y-6">
      <div className="neo-box-lg p-6 space-y-2">
        <div className="flex items-center gap-2">
          <span className="neo-sticker bg-zinc-950 text-white text-[10px]">Clipper Opportunities</span>
          <span className="neo-sticker bg-rose-100 text-rose-950 border-rose-950 text-[10px]">Active Sprints</span>
        </div>
        <h1 className="text-2xl sm:text-3xl font-['Unbounded'] font-black uppercase text-zinc-950 dark:text-white tracking-tight">
          Active Campaigns for Editors
        </h1>
        <p className="text-xs text-zinc-600 dark:text-zinc-400 font-medium">
          Select a campaign to download raw Google Drive source material, cut high-retention vertical edits, and submit links for verified CPM payout.
        </p>
      </div>

      {loading ? (
        <div className="neo-box p-12 text-center text-xs font-mono font-bold text-zinc-500 dark:text-zinc-400">Loading briefs...</div>
      ) : campaigns.length === 0 ? (
              ...
              <div className="pt-3 border-t-2 border-zinc-950 dark:border-zinc-700 space-y-4">
                <div className="grid grid-cols-2 gap-2 text-center text-xs font-mono">
                  <div className="p-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border-2 border-zinc-950 dark:border-zinc-700 shadow-[2px_2px_0px_#09090b]">
                    <span className="text-[10px] text-zinc-500 dark:text-zinc-400 block font-bold uppercase">CPM RATE</span>
                    <span className="font-black text-emerald-700 dark:text-emerald-400 text-sm">৳{c.cpmRate}/1k</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-800 border-2 border-zinc-950 dark:border-zinc-700 shadow-[2px_2px_0px_#09090b]">
                    <span className="text-[10px] text-zinc-500 dark:text-zinc-400 block font-bold uppercase">BUDGET LEFT</span>
                    <span className="font-black text-zinc-950 dark:text-white text-sm">৳{c.remainingBudget.toLocaleString()}</span>
                  </div>
                </div>
